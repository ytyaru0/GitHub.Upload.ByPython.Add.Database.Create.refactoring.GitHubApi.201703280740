...
#        self.db_lang = dataset.connect('sqlite:///' + path_languages_sqlite3)
    ...
    def Insert(self, y):
        self.data.db_lang.begin()
        count = 0
        for key in y.keys():
            print(key)
            print(y[key])
            if None is self.data.db_lang['Languages'].find_one(Key=key):
                self.data.db_lang['Languages'].insert(self.__CreateLanguages(key, y[key]))
                language_id = self.data.db_lang['Languages'].find_one(Key=key)['Id']
                ...
                count += 1
                print('重複レコード。')
                print(self.data.db_lang['Languages'].find_one(Key=key))
        self.data.db_lang.commit()
        print(count)
    ...
    def __InsertAliases(self, language_id, lang):
        ...
        for aliase in lang['aliase']:
            self.data.db_lang['Aliases'].insert(dict(
    ...
    def __InsertExtensions(self, language_id, lang):
        ...
        for extension in lang['extensions']:
            self.data.db_lang['Extensions'].insert(dict(
                LanguageId=language_id,
                Extension=extension,
    ...
    def __InsertFileNames(self, language_id, lang):
        ...
        for filename in lang['filenames']:
            self.data.db_lang['FileNames'].insert(dict(
                LanguageId=language_id,
                FileName=filename
            ))
    def __InsertInterpreters(self, language_id, lang):
        ...
        for interpreter in lang['interpreters']:
            self.data.db_lang['Interpreters'].insert(dict(
                LanguageId=language_id,
